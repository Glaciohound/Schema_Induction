...
from components.muc4_tools import \
    get_all_paragraphs, get_all_type_sentences, merge_sentences_to_paragraph, \
...
from components.prompt_all import prompt_all_paragraphs
...
def get_args():
    ...
    parser.add_argument(
        "--paragraphs-by-type", type=str,
        default="data/muc34/outputs/muc4-paragraphs-by-type.json")
...
def retrieve_by_type(events, corpora, args):
    ...
    # prompted_lists = prompt_all_paragraphs
    ...
    all_paragraphs = np.array(get_all_paragraphs(corpora))
    num_paragraphs = len(all_paragraphs)
    content_arg = "content-cased"
    type_paragraphs = {
        _type: [
            _sentence
            for _sentences in _group
            for _sentence in _sentences[:1]
        ]
        for _type, _group in
        get_all_type_sentences(events, corpora, content_arg).items()
    }
    type_gt_indexes = {
        _type: np.array([
            np.where(all_paragraphs == _sentence["sentence"])[0][0]
            for _sentence in _sentences
        ])
        for _type, _sentences in type_paragraphs.items()
    }
    with open(args.paragraphs_by_type, 'w') as f:
        json.dump(type_paragraphs, f, indent=2)
    exit()
